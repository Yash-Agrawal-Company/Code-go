...
export default function EditorPage() {
  ...
  const [code, setCode] = useState('// Welcome to CodeGo! \n// Start typing to collaborate in real-time.\n');
  ...
  useEffect(() => {
    ...
    socket.on('cursor-sync', ({ socketId, user: cursorUser, position }) => {
      ...
      const decorationTheme = cursorUser.name === 'Alice' ? 'cursor-pink' : 'cursor-blue';
      ...
      decorationsRef.current = editor.deltaDecorations(decorationsRef.current, [newDecoration]);
    });
    ...
    <div className="h-screen flex flex-col bg-slate-900 text-white overflow-hidden">
      ...
      <header className="h-16 border-b border-slate-800 flex items-center justify-between px-6 bg-slate-900/50 backdrop-blur-md">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/dashboard')} className="p-2 hover:bg-white/5 rounded-xl transition text-gray-400">
        ...
        <div className="flex items-center gap-6">
          <div className="flex -space-x-3 pr-4 border-r border-slate-700">
            {activeUsers.slice(0, 5).map((u, i) => (
              <div key={i} title={u.user?.name} className="w-8 h-8 rounded-full border-2 border-slate-900 bg-gradient-to-r from-blue-500 to-indigo-500 flex items-center justify-center text-xs font-bold shadow-lg z-10">
          ...
          <select 
            ...
            onChange={(e) => {
              ...
              if (socket) {
                ...
                socket.emit('save-document', { roomId, code, language: newLang });
              }
            }}
